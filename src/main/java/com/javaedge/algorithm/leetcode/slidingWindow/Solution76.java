...
public class Solution76 {
    public String minWindow(String s, String t) {
        ...
        // 开始滑动窗口的遍历过程
        // right作为滑动窗口的右指针，初始值为0，用于界定窗口的右边界
        // left作为滑动窗口的左指针，初始值为0，用于界定窗口的左边界
        // cnt用于记录当前窗口内已经匹配上目标字符串t中字符的个数
        for (int left = 0, right = 0, cnt = 0; right < n;right++) {
            // 将当前右指针right指向的字符chars[right]在hash数组中的值加1
            // 表示该字符在窗口内又出现了一次
            hash[chars[right]]++;
            ...
            while (cnt == m && hash[chars[left]] > 0) {
                // 将当前左指针left指向的字符chars[left]在hash数组中的值减1
                // 表示该字符在窗口内的出现次数减少一次，同时左指针left右移一位（通过left++）
            ...
            // 进一步判断是否需要更新最小覆盖子串
            if (cnt == m) {
                // 如果当前找到的最小覆盖子串res为空字符串或者当前窗口的长度（right - left + 1）小于res的长度
                // 就将res更新为当前窗口对应的子串，即通过s.substring(right, left + 1)来获取并赋值给res
